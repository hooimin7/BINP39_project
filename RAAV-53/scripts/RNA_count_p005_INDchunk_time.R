...
load("../../RAAV-60/p005/04_blast/02_analysis/multipleContfragmentsComplete.rda")
load("../../RAAV-60/p005/05_bowtie/alignedLibraries.rda")

# File paths for barcodes
barcodes.file <- "03_pairfq/barcodes_5_BarLib_paired_reads.fastq.gz"
...
# Reduce barcode
out.name.BC.star <- "05_starcode/01_data/barcodes_5_reduced.txt"
table.BC.sc <- data.table(read.table(out.name.BC.star, header = FALSE, row.names = 1, skip = 0, sep = "\t", stringsAsFactors = FALSE, fill = FALSE), keep.rownames = TRUE, key = "rn")
table.BC.sc[, `:=`(V2, NULL)]
...
chunk_size <- ceiling(nrow(output.Table) / 40)  # Adjust the chunk size to be smaller
...
for (chunk_index in seq_along(output_chunks)) {
  ...
  name.out <- paste0("p005_53_39_S14_L001_R1_001_chunk_", chunk_index)
  ...
  if (length(foundFragments.ranges) > 0) {
    ...
    saveRDS(foundFragments.ranges, file = paste("06_traceBack/03_p005_found/", "found.", name.out, ".rds", sep = ""), compress = TRUE)
  }
