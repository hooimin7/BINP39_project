...
process_directory <- function(dir) {
  ...
  out_dir <- file.path(dir, "06_traceBack", "04_p007_found")
  ...
  barcodes_file <- file.path(dir, "03_pairfq", "barcodes_7_BarLib_paired_reads.fastq.gz")
  starcode_file <- file.path(dir, "05_starcode", "01_data", "barcodes_7_reduced.txt")
  
  # Check if files exist
  if (file.exists(barcodes_file) && file.exists(starcode_file)) {
    # Load sequences
    LUT.dna <- fread("../RAAV-60/p007/04_blast/LUTdna.csv")
    load("../RAAV-60/p007/04_blast/02_analysis/multipleContfragmentsComplete.rda")
    ...
    # Split output.Table into smaller chunks
    chunk_size <- ceiling(nrow(output.Table) / 30)  # Adjust the chunk size to be smaller
    output_chunks <- split(output.Table, ceiling(seq_along(1:nrow(output.Table)) / chunk_size))

    # Serialize allFragments.ranges to avoid non-exportable references
    serialized_allFragments <- serialize(allFragments.ranges, NULL)

    # Setup parallel backend using future
    plan(multisession, workers = 4)  # Increase the number of workers to 4

    # Process each chunk sequentially
    for (chunk_index in seq_along(output_chunks)) {
      chunk <- output_chunks[[chunk_index]]
      ...
      foundFrags <- chunk[BCcount, nomatch = 0]
      ...
      print(paste("Processing chunk", chunk_index, "of", length(output_chunks)))
      ...
      matchRange <- function(idxFrag, serialized_allFragments) {
        allFragments.ranges <- unserialize(serialized_allFragments)
      ...
      time_parallel_processing <- system.time({
        match.ranges.list <- future_lapply(1:nrow(foundFrags), function(i) {
          matchRange(i, serialized_allFragments)
        })
      })
      print(paste("Time for parallel processing on chunk", chunk_index, ":", time_parallel_processing[3], "seconds"))
      ...
      print("First few elements of match.ranges.list:")
      print(head(match.ranges.list))
      ...
      foundFragments.ranges <- unserialize(serialized_allFragments)[match.ranges[, 1]]

      # Check if foundFragments.ranges is not NULL
      if (is.null(foundFragments.ranges)) {
        print("foundFragments.ranges is NULL.")
        next
      }
      ...
      name.out <- paste0('p007_72_chunk_', chunk_index)

      # If there are more than one match, then save the found fragments
      if (length(foundFragments.ranges) > 0) {
