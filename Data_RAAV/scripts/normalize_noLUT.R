...
loadRDS <- function(in.name) {
  ...
  if (inherits(this.sample, c("data.table", "data.frame"))) {
...
# Combine the list of R objects into a single data.table
all.samples <- rbindlist(all.samples, fill = TRUE)

print("all.samples after combining")
print(head(all.samples))
...
all.samples[, `:=`(c("BC.1", "tCount.1", "Mode.1", "mCount.1"), NULL)]
...
setkeyv(all.samples,c("Group","Reads"))
...
all.samples[, Peptide := future_lapply(Reads, translate_function)]
