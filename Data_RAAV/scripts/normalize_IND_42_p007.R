...
file1 <- file.path(base_dir, "02_rds", "found.p007_53_42_S15_L001_R1_001.rds")
...
normalize_chunk <- function(chunk_index, file1_data, GENETIC_CODE_ALT) {
  file2_chunk <- file.path(base_dir, "02_rds", "01_chunkLib", paste0("output_chunks_", chunk_index, ".rds"))
  file3_chunk <- file.path(base_dir, "02_rds", "02_chunkAAV", paste0("found.p006_AAV_02_chunk_", chunk_index, ".rds"))
  ...
  saveRDS(all.samples, file = paste0("03_normalize/42_S15/allSamplesDataTable_p007_42_chunk", chunk_index, ".RDS"))

  return(all.samples)
}

# Process each chunk
num_chunks <- 29  # Total number of chunks
all_normalized_data <- list()

for (i in 1:num_chunks) {
  normalized_data <- normalize_chunk(i, file1_data, GENETIC_CODE_ALT)
  all_normalized_data[[i]] <- normalized_data
}

# Combine all normalized data
chunk_files <- list.files("03_normalize/42_S15", pattern = "allSamplesDataTable_p007_42_chunk.*\\.RDS", full.names = TRUE)
all_normalized_data <- lapply(chunk_files, readRDS)
final_normalized_data <- rbindlist(all_normalized_data)

# Save the final combined normalized data
saveRDS(final_normalized_data, file = "03_normalize/42_S15/allSamplesDataTable_final_p007_42.RDS")

# Load the final combined normalized data
final_normalized_data <- readRDS("03_normalize/42_S15/allSamplesDataTable_final_p007_42.RDS")
...
readCounts <- final_normalized_data[, list(GroupCount = sum(RNAcount)), by = "Group"]
...
final_normalized_data <- rbind(final_normalized_data, total.AAV.samples, transported.AAV.samples.53)

# Remove the data.tables that were created before
rm(total.AAV.samples, transported.AAV.samples.53)
...
saveRDS(final_normalized_data, file = "03_normalize/42_S15/allSamplesDataTable_final_normalized_p007_53_42.RDS")