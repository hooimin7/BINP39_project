...
for (dir in directories) {
  ...
  rds_files <- list.files(path = dir, pattern = "_normalized_p007_53.*\\.RDS$", full.names = TRUE)
...
select.samples <- combined_data[J(c("mRNA_53", "mRNA_All"))]
...
plotPair <- function(topSample, bottomSample, size.bin = 1, winWidth = 1, NormalizePlot = TRUE, PlotBC = FALSE) {
  ...
  # Extract and process topSample and bottomSample
    ...
    ggsave(paste0("../04_plot/top25_AAV_lib_test/combined_plot_53AAV_", top_genes[i], ".png"), plot = list_plots[[i]], width = 8, height = 6, dpi = 300, units = "in")
    }

    # Optionally, combine all plots into a single PDF
    pdf("../04_plot/top25_AAV_lib_test/combined_plots_53AAV.pdf", width = 8, height = 6)
    for (i in seq_along(list_plots)) {
        print(list_plots[[i]])
    }
    dev.off()
