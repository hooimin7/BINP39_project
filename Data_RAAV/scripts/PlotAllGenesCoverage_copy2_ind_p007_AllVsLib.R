  ...
  for (rds_file in rds_files) {
    ...
    plotPair <- function(topSample, bottomSample, size.bin = 1, winWidth = 1, NormalizePlot = TRUE, PlotBC = FALSE) {
      ...
      # select the top 200 samples
        select.samples.top <- select.samples.binPos[, head(.SD, 200), by=Group]
        ...
        # Extract the gene names of the top 200 samples
        top_genes <- unique(select.samples.top$GeneName)
    
        # Print the top 200 gene names for verification
        print("Top 200 gene names:")
        print(top_genes)
    
        # Filter plot.data.bin for the top 200 genes
        ...
        ggsave(paste0("../04_plot/top25_AAV_lib_test/", basename(dir), "_plot_72AAVlib", top_genes[i], ".png"), plot = list_plots[[i]], width = 8, height = 6, dpi = 300, units = "in")
        }
    
        # Optionally, combine all plots into a single PDF
        pdf(paste0("../04_plot/top25_AAV_lib_test/", basename(dir), "_combined_plots_72AAVlib.pdf"), width = 8, height = 6)
        for (i in seq_along(list_plots)) {
            print(list_plots[[i]])
        }
        dev.off()
