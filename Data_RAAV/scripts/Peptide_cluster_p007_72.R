...
for (subdir in subdirectories) {
    ...
    output_dir <- file.path("06_motif_72", base_name)
    dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
    
    # List RDS files matching the pattern in the current subdirectory
    rds_files <- list.files(path = subdir, pattern = "_normalized_p007_72.*\\.RDS$", full.names = TRUE)
    ...
    for (rds_file in rds_files) {
      ...
      select.samples <- all.samples[J(c("library", "mRNA_All", "mRNA_72"))] 
      ...
      hammock_cmd <- paste(
        "java -jar /home/hooimin/Hammock_v_1.2.0/Hammock_v_1.2.0/dist/Hammock.jar full -i", fasta_file_path, "-d", hammock_output_dir, "--max_shift 7 -c 65 -t", detectCores(),
        sep = " "
      )
      ...
      generateWeblogo <- function(in.name) {
        ...
        cat('\n')
        cat('\n')
        cat(paste0("![Peptide: ", this.main$main_sequence, " from ", main.gene, " with cluster number ", in.name, "](06_motif_72/WEBlogos/", in.name, ".pdf)"))
        cat('\n')
